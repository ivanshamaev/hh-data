"""
DAG: hh_vacancies_spb

Загружает вакансии HH.ru по Санкт-Петербургу для каждой профессиональной роли из raw.roles.
Роли берутся из БД. Та же логика, что у hh_vacancies_moscow (merge в raw.vacancies).

Требуется Airflow Variable: token (Bearer-токен HH.ru).
Опционально: hh_user_agent (по умолчанию hh_data/1.0).
"""
...
AREA_SPB = 2  # Санкт-Петербург в API HH.ru
PER_PAGE = 100
REQUEST_DELAY_S = 0.2
REQUEST_RETRIES = 4
...
def _fetch_and_insert_vacancies_for_role(
    ...
    **kwargs,
    ...
    """Для одной роли: запрос к API по страницам (СПб), разбор через Polars, merge в raw.vacancies."""
    import json
    ...
    if not all_items:
        logging.info("Role %s (SPb): 0 vacancies, nothing to merge", role_id)
        return
    ...
    logging.info("Role %s (SPb): merged %d vacancies", role_id, len(df))
...
with DAG(
    dag_id="hh_vacancies_spb",
    start_date=datetime(2026, 3, 1),
    schedule_interval=None,
    catchup=False,
    max_active_runs=1,
    tags=["ingestion", "hh", "vacancies", "spb"],
) as dag:
