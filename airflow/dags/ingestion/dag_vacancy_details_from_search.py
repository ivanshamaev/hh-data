"""
DAG: vacancy_details_from_search

Загружает детали вакансий по API GET /vacancies/{id} только для вакансий из view raw.vacancies_search_dedup
(дедуплицированный по id поиск). Дельта: только id, которых ещё нет в vacancy_details_loaded.
...
def _fetch_and_save_vacancy_details(conn_id: str = DEFAULT_CONN_ID) -> None:
    ...
    cur.execute(
        ...
        FROM raw.vacancies_search_dedup v
...
with DAG(
    dag_id="vacancy_details_from_search",
    start_date=datetime(2026, 3, 1),
    schedule_interval=None,
    catchup=False,
    max_active_runs=1,
    tags=["ingestion", "hh", "vacancy_details", "vacancies_search"],
) as dag:
