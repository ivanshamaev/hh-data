...
-- Hash key = dv.md5_as_uuid(business_key). ON CONFLICT DO NOTHING для идемпотентности.

INSERT INTO dv.H_Vacancy (vacancy_hk, vacancy_id, load_dt, record_source)
SELECT dv.md5_as_uuid(d.id), (d.id)::bigint, COALESCE(d.inserted_at, now()), 'raw.vacancy_details'
FROM raw.vacancy_details d
WHERE d.id ~ '^\d+$'
ON CONFLICT (vacancy_hk) DO NOTHING;

INSERT INTO dv.H_Area (area_hk, area_id, load_dt, record_source)
SELECT dv.md5_as_uuid(area_id), area_id::int, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(employer_id), employer_id::bigint, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(schedule_id), schedule_id, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(employment_id), employment_id, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(experience_id), experience_id, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(billing_type_id), billing_type_id, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(employment_form_id), employment_form_id, min(inserted_at), 'raw.vacancy_details'
FROM (
...
-- Профессиональные роли: из массива professional_roles[].id (в API — целое число)
INSERT INTO dv.H_ProfessionalRole (professional_role_hk, professional_role_id, load_dt, record_source)
SELECT dv.md5_as_uuid(role_id), role_id::int, min(inserted_at), 'raw.vacancy_details'
FROM (
...
SELECT dv.md5_as_uuid(skill_name), skill_name, min(inserted_at), 'raw.vacancy_details'
FROM (
